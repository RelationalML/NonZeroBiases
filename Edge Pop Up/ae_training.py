...
def rescaling(model, device, data, target, criterion):
    ...
    for i in range(20):
        ll = criterion(scale*output, data)
        opt.zero_grad()
        ll.backward()
        opt.step()
    depth = 0
...
def trainReg(model, scheduler, device, train_loader, optimizer, criterion, epoch, log_interval, scaling, args):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # print(data.shape, output.shape)
        #print(output.size())
        # Assuming MSE Loss for reconstruction
        loss = criterion(output.view(data.shape), data)
        loss.backward()
        optimizer.step()
        scheduler(epoch, batch_idx)
        if batch_idx % log_interval == 0:
            ...
            train_loss = loss.item()
    if scaling:
        model = rescaling(model, device, data, target, criterion)
...
def testReg(model, device, criterion, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.to(device), target.to(device=device, dtype=torch.float)
            output = model(data)
            test_loss += criterion(output.view(data.shape), data).item() * data.size(0)
            #pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
            #correct += pred.eq(target.view_as(pred)).sum().item()
